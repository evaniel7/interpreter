    ...
    (return-value*
      ...
      (lambda (instance post-eval-layers)
        (let* ((runtime-class (car instance))
               (field-vals (cadr instance))
               (inherited-fields (get-inherited-fields runtime-class post-eval-layers))
               (field-names (map name inherited-fields)))
          (cond
            ((var-exists? fieldname field-vals)
             (cc (get-variable fieldname field-vals) post-eval-layers))

            ((member fieldname field-names)
             (error
              (string-append
               "Instance of class \""
               (symbol->string runtime-class)
               "\" lost inherited field \""
               (symbol->string fieldname)
               "\". Check call-this-function updated-fields reconstruction.")))
            ...
            (else
             (error
              (string-append
               "Field \""
               (symbol->string fieldname)
               "\" does not exist in this class instance!"))))))
      throw)))
